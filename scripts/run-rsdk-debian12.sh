#!/bin/sh
set -e
# run-rsdk-debian12: run the installed offline image with sensible mounts
# Usage: run-rsdk-debian12 [--name NAME] [--] [docker run args...]

IMAGE_HINT="rsdk-debian12"
ARCH=$(dpkg --print-architecture 2>/dev/null || echo amd64)
IMG_TAR="/usr/share/rsdk-debian12/images/${ARCH}/image.tar"
